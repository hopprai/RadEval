...
class HopprCTCLungNodulesMetric(MetricBase):
    ...
    def metric_keys(self, detailed: bool = False) -> list[str]:
        keys = list(self._all_aggregate_keys())
        if detailed:
    ...
    def _per_sample_output(df: pd.DataFrame) -> dict[str, Any]:
        """Per-sample mode: return one list per legacy rate key.
        ...
        for out_key, col in _LEGACY_RATE_KEYS.items():
            out[out_key] = df[col].tolist() if col in df.columns else []
        return out

    def _aggregate_output(self, df: pd.DataFrame, detailed: bool) -> dict[str, Any]:
        out: dict[str, Any] = {}
        # Legacy rate keys: mean of per-row rates.
        for out_key, col in _LEGACY_RATE_KEYS.items():
        ...
        # Per-bucket sensitivity and FP-per-study (corpus-level rates).
        for b in SIZE_BUCKETS:
            ...
            out[f"hoppr_ctc_lung_nodules_sensitivity_{b}"] = (
                round(sens, 4) if sens is not None else None
            )
            out[f"hoppr_ctc_lung_nodules_fp_per_study_{b}"] = (
                round(fp_per_study, 4) if fp_per_study is not None else None
            )

        # Per-bucket diameter MAE (4to6, 6to8) and MAPE (>8 mm aggregated).
        for b in ("4to6", "6to8"):
            ...
            out[f"hoppr_ctc_lung_nodules_diameter_mae_{b}"] = (
                round(mae, 4) if mae is not None else None
            )
        # >8 mm MAPE pools 8to15 and gt15.
        gt8_num = float(
        ...
        out["hoppr_ctc_lung_nodules_diameter_mape_gt8"] = (
            round(mape_gt8, 4) if mape_gt8 is not None else None
        )

        # Density two-class accuracy (corpus sums).
        ...
        out["hoppr_ctc_lung_nodules_density_acc_solid"] = (
            round(acc_solid, 4) if acc_solid is not None else None
        )
        out["hoppr_ctc_lung_nodules_density_acc_subsolid"] = (
            round(acc_subsolid, 4) if acc_subsolid is not None else None
        )
        if acc_solid is not None and acc_subsolid is not None:
            out["hoppr_ctc_lung_nodules_density_acc_macro"] = round(
                (acc_solid + acc_subsolid) / 2.0, 4
            )
        elif acc_solid is not None:
            out["hoppr_ctc_lung_nodules_density_acc_macro"] = round(acc_solid, 4)
        elif acc_subsolid is not None:
            out["hoppr_ctc_lung_nodules_density_acc_macro"] = round(acc_subsolid, 4)
        else:
            out["hoppr_ctc_lung_nodules_density_acc_macro"] = None

        # Calcified, laterality, lobe accuracy.
        for out_key, num_col, den_col in [
            ...
            out[out_key] = round(acc, 4) if acc is not None else None
