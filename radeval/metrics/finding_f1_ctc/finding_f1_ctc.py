...
CLINICAL_WEIGHTS: dict[str, int] = {
    ...
    "acute_aortic_injury": 5,
    "acute_pulmonary_embolism": 5,
    "pneumothorax": 5,
    ...
    "bronchiectasis": 2,
    "cardiomegaly": 2,
    "chronic_pulmonary_embolism": 2,
    "copd_emphysema": 2,
    "interlobular_septal_thickening": 2,
    "vertebral_compression_fracture": 2,
    # Tier 1 — Low/incidental: degenerative, rarely changes management
    "aortic_atherosclerosis": 1,
    "aortic_valve_calcification": 1,
    "coronary_artery_calcifications": 1,
    "prior_myocardial_infarction": 1,
    "neck_base": 1,
    "upper_abdomen": 1,
}
...
class FindingF1CTC:
    ...
    def __call__(
        ...
        # Weighted macro-average: each finding's F1 is weighted by clinical urgency
        total_weight = sum(CLINICAL_WEIGHTS.get(f, 1) for f in per_finding_metrics)
        weighted_macro_f1 = (
            sum(CLINICAL_WEIGHTS.get(f, 1) * m["f1"] for f, m in per_finding_metrics.items()) / total_weight
            if total_weight else 0.0
        )
        weighted_macro_sens = (
            sum(CLINICAL_WEIGHTS.get(f, 1) * m["sensitivity"] for f, m in per_finding_metrics.items()) / total_weight
            if total_weight else 0.0
        )
        weighted_macro_spec = (
            sum(CLINICAL_WEIGHTS.get(f, 1) * m["specificity"] for f, m in per_finding_metrics.items()) / total_weight
            if total_weight else 0.0
        )
        weighted_macro_ppv = (
            sum(CLINICAL_WEIGHTS.get(f, 1) * m["ppv"] for f, m in per_finding_metrics.items()) / total_weight
            if total_weight else 0.0
        )
        weighted_macro_npv = (
            sum(CLINICAL_WEIGHTS.get(f, 1) * m["npv"] for f, m in per_finding_metrics.items()) / total_weight
            if total_weight else 0.0
        )
        ...
        # Weighted micro-average: TP/FP/FN/TN scaled by clinical weight before aggregation
