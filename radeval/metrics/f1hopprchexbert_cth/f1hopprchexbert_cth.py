"""HopprF1CheXbertCTH: multi-output CT-head report evaluator (ModernBERT-large).

A single forward pass classifies 10 CT-head incidental/chronic findings
simultaneously. Each finding head outputs 3-way logits (0 = absent or not
reported, 1 = present, 2 = uncertain), collapsed to binary for F1 evaluation:
positive = {present, uncertain} (i.e. predicted class != 0), negative = absent.

Mirrors the f1hopprchexbert_ct metric, adapted to the 3-class label scheme used
by the text_classifiers CTH system (src/text_classifier/labels.py).
"""
    ...
    def __init__(
        ...
        if not os.path.isdir(checkpoint_dir):
            raise FileNotFoundError(
                f"HopprF1CheXbertCTH checkpoint not found: {checkpoint_dir}")
