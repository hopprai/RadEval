"""HopprF1MskRrgLv005ClsLv009: full-body MSK report evaluator (ModernBERT-large).

A single forward pass classifies 26 full-body MSK findings simultaneously.
Each finding head outputs 3-way logits (0 = absent or not reported, 1 = present,
2 = uncertain), collapsed to binary for F1 evaluation: positive =
{present, uncertain} (predicted class != 0), negative = absent.

Trained on msk_rrg_internal_lv005 (clean_findings) + msk_cls_internal_lv009
labels, prevalence-subsampled 3.04M -> 750K studies. Metric name follows the
input-based convention f1hoppr_<rrg_version>_<cls_version>.
"""
...
_DEFAULT_CKPT = (
    "/nfs/cluster/hoppr_vlm_ressources/radeval_checkpoints/f1hoppr_msk_rrg_lv005_cls_lv009"
)

CONDITION_NAMES = OrderedDict([
    ("acute_fracture", "Acute fracture"),
    ("healed_fracture", "Healed fracture"),
    ("osteoarthritis", "Osteoarthritis"),
    ("osteophyte_formation", "Osteophyte formation"),
    ("joint_space_narrowing", "Joint space narrowing"),
    ("subchondral_change", "Subchondral change"),
    ("degenerative_spine_disease", "Degenerative spine disease"),
    ("disc_space_narrowing", "Disc space narrowing"),
    ("foraminal_narrowing", "Foraminal narrowing"),
    ("spondylolisthesis", "Spondylolisthesis"),
    ("scoliosis", "Scoliosis"),
    ("abnormal_spinal_curvature", "Abnormal spinal curvature"),
    ("joint_subluxation_dislocation", "Joint subluxation or dislocation"),
    ("joint_effusion", "Joint effusion"),
    ("soft_tissue_swelling", "Soft tissue swelling"),
    ("soft_tissue_calcification", "Soft tissue calcification"),
    ("osteopenia", "Osteopenia"),
    ("bone_lesion", "Bone lesion"),
    ("erosion", "Erosion"),
    ("hardware_present", "Hardware present"),
    ("arthroplasty", "Arthroplasty"),
    ("spinal_fusion", "Spinal fusion"),
    ("avascular_necrosis", "Avascular necrosis"),
    ("chondral_lesion", "Chondral lesion"),
    ("accessory_ossicle", "Accessory ossicle"),
    ("spine_developmental_variant", "Spine developmental variant"),
])
...
class MultiOutputClassifier(PreTrainedModel):
    """26 independent 3-class heads sharing one BERT-style encoder."""
...
class HopprF1MskRrgLv005ClsLv009:
    """Multi-output full-body MSK finding classifier for report evaluation.

    A single forward pass produces (batch, 26, 3) logits. Predictions are
    ...
    def __init__(
        ...
        if not os.path.isdir(checkpoint_dir):
            raise FileNotFoundError(
                f"HopprF1MskRrgLv005ClsLv009 checkpoint not found: {checkpoint_dir}")
    ...
    def _predict_label_matrix(
        ...
        """Return binary label matrix of shape (N, 27).

        Logits (N, 26, 3) -> argmax -> binary (classes 1, 2 = positive).
        A 27th "no_finding" column is 1 when all 26 findings are 0.
        """
        all_binary = []
        ...
        for start in range(0, len(report_list), self.batch_size):
            ...
            logits = self.model(**enc).logits  # (B, 26, 3)
            pred_ids = logits.argmax(dim=-1)   # (B, 26)
            binary = (pred_ids != 0).int().cpu()  # present or uncertain -> positive
            all_binary.append(binary)
            if on_batch_done:
                on_batch_done()

        matrix = torch.cat(all_binary, dim=0)  # (N, 26)
