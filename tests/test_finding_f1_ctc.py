...
def test_finding_f1_ctc_basic():
    """Test basic finding F1 extraction on small CT chest examples."""
    from radeval import RadEval

    refs = [
        "The lungs are clear. No pleural effusion or pneumothorax. Heart size is normal.",
        "There is a moderate left pleural effusion. Aortic atherosclerosis is present.",
    ]
    hyps = [
        "Clear lungs bilaterally. No effusion or pneumothorax identified. Cardiac silhouette is within normal limits.",
        "Left pleural effusion is noted. Atherosclerotic aortic calcifications present.",
    ]

    evaluator = RadEval(metrics=["finding_f1_ctc"])
